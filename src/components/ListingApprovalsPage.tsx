...
import { ClockIcon, CheckCircleIcon, XCircleIcon, InfoIcon, SearchIcon, FilterIcon, ChevronDownIcon } from 'lucide-react';
...
export const ListingApprovalsPage: React.FC = () => {
  ...
  const summaryData = [{
    ...
    count: displayServices.filter(service => service.status === 'Approved').length,
    ...
    id: 'rejected',
    title: 'Rejected',
    count: displayServices.filter(service => service.status === 'Rejected').length,
    icon: XCircleIcon,
  ...
  const renderExpandedContent = (row: any) => {
    const originalService = displayServices.find(service => service.id === row.id);
    return <div className="p-4 bg-gray-50 rounded-md">
        <h4 className="font-medium text-gray-900 mb-2">Service Preview</h4>
        <p className="text-gray-700 mb-3">
          {originalService?.description || 'No description available.'}
        </p>
  ...
  const handleApproveService = () => {
    if (selectedService) {
      // Approve and publish the service
      alert(`Service "${selectedService.title}" approved!`);
      setShowApproveModal(false);
      setIsDrawerOpen(false);
    }
  };
  const handleRejectService = (reason: string) => {
    if (selectedService) {
  ...
  const handleSendBackService = (reason: string, comments: string) => {
    if (selectedService) {
      ...
      <div className="bg-white rounded-xl shadow-md p-4 mb-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <div className="relative inline-block">
              <select className="appearance-none bg-white border border-gray-300 rounded-md py-2 pl-3 pr-8 text-sm leading-5 focus:outline-none focus:ring-blue-500 focus:border-blue-500" value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)}>
