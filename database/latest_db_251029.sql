...
COMMENT ON FUNCTION "public"."increment_flag_count"("content_id" "uuid") IS 'Atomically increments the flag_count for a content item';
...
COMMENT ON COLUMN "public"."auth_user_profiles"."role" IS 'User role: admin (full management), editor (create/update), approver (review/approve), viewer (read-only). Defines what the user can do.';



COMMENT ON COLUMN "public"."auth_user_profiles"."user_segment" IS 'User segment: internal (platform staff), partner (service providers), customer (end users), advisor (consultants). Defines who the user is.';
...
COMMENT ON COLUMN "public"."cnt_contents"."flag_count" IS 'Number of times content has been flagged for review';



COMMENT ON COLUMN "public"."cnt_contents"."flagged_at" IS 'Timestamp when content was last flagged for review';
...
COMMENT ON POLICY "Allow profile lookup during login" ON "public"."auth_user_profiles" IS 'Permissive policy for login flow - allows unauthenticated profile lookups. Should be restricted in production.';
...
COMMENT ON POLICY "Users can view their own profile" ON "public"."auth_user_profiles" IS 'Allows users to view their own profile or service role to view any profile';
